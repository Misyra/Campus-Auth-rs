...
## 开发中（2026-09-26 浏览器默认任务绑定状态文案）

### 背景

- 用户反馈：浏览器任务列表把内置 `default` 任务标成「未绑定」，编辑页却说明它是「内置默认任务：方案未选中任何任务时浏览器登录用它」——绑定索引只回答"有没有方案显式引用"，渲染成「未绑定」会被读成"这条任务没人用"，两处自相矛盾。

### 前端

- `views/tasks/BrowserTasksPanel.vue`：列表「绑定方案」列与编辑态侧栏的绑定区，`default` 在无任何方案显式绑定时改显「内置兜底」徽章（`badge--info`，title 说明兜底语义），替换「未绑定」/「尚未被任何方案引用」；被显式绑定时照常显示方案名 pills，零引用的普通任务仍显「未绑定」。直连/脚本面板不动：后端把 `default` ID 保留给浏览器任务（`tasks/loader.rs` 保存校验拒绝非浏览器类型占用），那两类没有对应歧义。
