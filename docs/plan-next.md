...
**修掉的一个真缺陷（F1，高）**：脚本渠道**在默认配置下完全跑不起来**。方案编辑器把「执行程序 = Python (项目内解释器)」存成**空串**（不是 `null`），而 `build_script_command` 只判 `None`，`Some("")` 被当成真实路径 → `Command::new("")` → `IO 错误: program path has no file name`。真机三连验证：默认解释器「立即运行」失败；方案绑该任务后 `POST /api/login` 0.2 秒终态失败；把解释器改成下拉里的具体 python 路径后同一方案 5.4 秒登录成功。**为什么测试全绿却没拦住**：三份 fixture 写的都是 `"binary_path": null`（走 `None` 分支），且没有任何测试真的跑过一次脚本任务——UI 写出的空串形态从未进入执行路径。修复：新增 `explicit_binary()`（空白串一律按未指定），四个分支统一走它，并使「解析出的程序名为空」提前返回可诊断的中文报错；补 3 例回归。

**顺手收掉的三项**：脚本页两处「脚本不参与登录认证」的过时文案（与 `去新建 →` 引导到同一页自相矛盾）；删除定时任务残留 `tasks/scheduled/history/{id}.json` 成孤儿文件（现连同历史一并清理，补集成断言）；`useScripts.ts` 同源过时注释。

**本轮登记的待办**：

- **登录历史不记渠道**（既有项，与上方「脚本渠道」一节的同一条）：三渠道只能从消息文本辨认，建议给历史记录加 `channel` 字段（`#[serde(default)]` 兼容旧文件）+ 列表渲染渠道徽标。
