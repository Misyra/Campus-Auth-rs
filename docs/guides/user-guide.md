...
> 适用于 Rust 重写版 `campus-auth`（`v5.0.2`，单 binary + Python Worker 子进程）。Python 版 `main.py` / `start.exe` / `update.exe` 已不在本仓库出现，本文已按当前实现重写。
...
地址：`http://127.0.0.1:50721`（`app.port`；本地端口被占用或被 Windows 保留时由系统自动分配可用端口，实际地址见启动日志；Docker 默认固定监听 `0.0.0.0:50721`）。首次启动走初始配置向导（见下节）。
...
| **设置** | 检测 / 浏览器 / 任务与环境 / 系统与更新 / 外观 | `config/settings.json`，`/api/config` |
| 关于 | —（版本、更新与卸载） | — |
...
全新安装（尚未同意过协议）首次打开控制台时，会进入一份五步配置向导；第 2 步起可随时「跳过向导」，跳过或中断后可随时在「方案」「设置」页手动完成对应配置。

1. **使用协议**：阅读并勾选同意《使用协议》和《免责声明》。同意即完成向导的准入标记，之后不再重复弹出。
2. **登录方式**：选「浏览器自动化」（需要 Python 与浏览器，适合有验证码、动态表单的门户）或「直连请求」（免 Python 与浏览器，适合登录接口可直接调用、无验证码的门户）。不确定时先选浏览器自动化。
3. **环境准备**（仅浏览器渠道）：自动检测 uv / Python / 认证核心 / Playwright 引擎的就绪状态，未就绪可一键自动安装（首次安装耗时数分钟，取决于网络）；同时列出系统中探测到的浏览器供选择，也可点击安装 Playwright 托管引擎（Chromium/Firefox/WebKit），Edge/Chrome 未安装时给出官网下载入口。此步可跳过，稍后在「设置 · 任务与环境 / 浏览器」继续。
4. **任务匹配**：输入学校名称后，程序会对 GitHub 与 Gitee 两个任务仓库**并行测速**，自动选用更快的可用源拉取任务索引，再按学校名查找匹配的社区任务：找到匹配即列出条目，点「查看并导入」复用仓库导入弹窗完成导入，导入的任务自动绑定到当前方案；没有匹配时，浏览器渠道会回退到内置的默认登录任务，直连渠道可「浏览全部任务」手动导入或稍后再配。匹配按连续子串进行，搜学校缩写（如「电子科大」）可能搜不到全名条目，换个说法或浏览全部即可。
5. **完成**：自动切换到「调试模式」（浏览器窗口可见、DEBUG 日志、启动后不自动登录），便于完成剩余配置并验证登录。到「方案」页填写校园网账号、密码与认证地址，确认能正常登录后，到「设置 · 系统」的运行模式里切回**默认模式**即可日常使用。
...
- 托盘菜单自上而下：**状态**（引擎 · 网络，登录进行中时追加登录态；只读信息行，引擎运行中为正常深色、未运行时灰化）、分隔线、**启动监测 / 停止监测**（随引擎状态切换文本）、**手动登录**（等同控制台「手动登录」按钮，来源标记为手动）、**打开控制台**、**退出**；轻量模式支持按需唤醒 Web 控制台。左键单击托盘图标直接打开控制台（`src/tray`）。托盘不再提供更新检查入口，更新走关于页或设置 · 系统与更新。
...
下载慢、代理不稳，或手里已经有一个现成的包时，可以不走联网下载更新。**设置 · 系统与更新**页提供两种方式：
...
「关于」页底部的「卸载」是**真卸载**：程序目录本身也会被删掉，不需要你事后手动清理文件夹。

点「卸载」后弹窗会先探测并**逐项列出**将删除的内容（程序目录、`config/`、`tasks/`、`logs/`、`environment/`、`update/`，含各自的完整路径与是否存在），另有一份系统残留清单（开机自启动、`~/.campus_network_auth` 加密密钥目录、Playwright 浏览器缓存）。两个选项值得注意：

- **「保留配置与任务」**（默认**不勾**）：勾上后上面那几项用户数据整棵保留，程序目录里**除这些数据目录之外的文件照删**（含随包附带的 `resources/`、`docs/`、`python_worker/` 与源码副本，以及你自己放在这个目录里的其它文件）——所以不要把程序放进一个还装着别的东西的文件夹里。同时**加密密钥目录 `~/.campus_network_auth` 也会一并保留**：方案密码在配置里是密文，密钥一删它们就再也解不开（重装后得把所有方案密码重填一遍）。Playwright 浏览器缓存属系统残留，仍会被清理。
- **程序目录整体删除**：不勾选「保留配置与任务」时，程序目录连同其中**所有**文件一起删除，加密密钥目录与浏览器缓存也一并清理。

确认后程序会立即退出，删除由 `campus-auth-helper` 在退出后完成（Windows 上运行中的程序无法删除自己，程序文件与用户数据又在同一目录里）。**完成后会弹出一个系统提示框**，列出删掉了什么、以及没能删掉的项（通常是文件正被别的程序占用），后者可手动删除。日志在系统临时目录的 `campus-auth-uninstall.log`。

两种情况下卸载会被**拒绝**并说明原因：程序目录看起来是源码仓库（含 `.git` 或 `Cargo.toml`，避免把开发目录当成安装目录删掉），或目录下找不到主程序。
