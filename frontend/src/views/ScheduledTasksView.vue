...
<template>
  <div class="page-content scheduled-tasks-page">
        ...
        <div v-else class="task-list">
          <div v-for="task in st.scheduledTasks.value" :key="task.id" class="task-item hover-lift scheduled-task-item" :class="{ disabled: !task.enabled }">
            ...
            <div class="task-actions">
              <button class="btn btn-sm" @click="st.runScheduledTask(task.id)" title="手动执行">运行</button>
    ...
    <!-- 新建/编辑弹窗 -->
    <div v-if="st.showScheduledTaskModal.value" class="modal-overlay" @click.self="st.closeScheduledTaskModal()">
      <div class="modal-container" role="dialog" aria-modal="true" aria-label="定时任务编辑">
        <div class="modal-header">
          <h3>{{ st.editingScheduledTask.value ? '编辑定时任务' : '新建定时任务' }}</h3>
          <button class="btn btn-icon-only" @click="st.closeScheduledTaskModal()">
    ...
    <!-- 执行历史弹窗 -->
    <div v-if="st.selectedScheduledTaskId.value" class="modal-overlay" @click.self="st.closeScheduledTaskHistory()">
      <div class="modal-container modal-lg" role="dialog" aria-modal="true" aria-label="执行历史">
        <div class="modal-header">
          <h3>执行历史</h3>
          <button class="btn btn-icon-only" @click="st.closeScheduledTaskHistory()">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
              <line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/>
            </svg>
          </button>
