    ...
    async fn try_retry(
        ...
        *attempts_used += 1;
        ...
        if force_recycle {
            ...
            // `Cancelled` 摧毁对方在途执行（`force_recycle` 先 `trigger_all` 取消
            // 会话区 token，用户可见「请求已取消」而非"Worker 崩溃"），症状是
            // "任务自己失败"，根因却在登录重试上。故先校验槽位是否空闲或仍属
            // 本会话，否则跳过回收：本轮登录已失败，不回收最多让下一轮重新 spawn 一次。
